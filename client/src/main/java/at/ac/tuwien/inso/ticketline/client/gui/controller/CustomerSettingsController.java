...
/**
 * Controller for customer overview, user are able to edit and create customer
 */
@Component
public class CustomerSettingsController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resBundle) {
        ...
        tableCustomerView.getSelectionModel().selectedItemProperty().addListener((observable, oldValue, newValue) -> handleSelectionChange());
    ...
    private void handleSelectionChange() {
        if (tableCustomerView.getItems()!=null && tableCustomerView.getSelectionModel().getSelectedItem()!=null) {
    ...
    public void handleReload() {
        btnEditCustomer.setDisable(true);
        tableCustomerView.getSelectionModel().clearSelection();
        if (getCurrentPage() == 0) {
            if (getTotalPages() > 0) {
                setPages(1);
                loadPage(1);
